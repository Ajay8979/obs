...
public class PassportDaoImplTest {
	...
	public void updateNegativeTest() throws SQLException {
		...
		passportRequest.setSessionId("1214");
		int[] count= {0,0,0};
		when(jdbcTemplate.batchUpdate(Mockito.anyString(),Mockito.anyList())).thenReturn(count);
		status=passportDaoImpl.updatePassport(passportRequest);
		assertEquals(false ,status );
		}
	/*
	 ...
	 * thenReturn(update); status=passportDaoImpl.deletePassport(passportRequest);
	 * assertEquals(true,status ); }
	 */

	/*
	 * @Test public void deleteNegativeTest() throws SQLException {
	 * passportRequest=new PassportRequest();
	 * passportRequest.setPassportList(getPassport());
	 * passportRequest.setTransaactionType("delete");
	 * passportRequest.setSessionId("1214"); int[] count= {0,0,0};
	 * when(jdbcTemplate.batchUpdate(Mockito.anyString(),Mockito.anyList())).
	 * thenReturn(count); status=passportDaoImpl.deletePassport(passportRequest);
	 * assertEquals(false,status ); }
	 */
	@Test
	...
	passportRequest=new PassportRequest();
	passportRequest.setPassportList(getPassport());
	when(jdbcTemplate.query("fghfgh",new BeanPropertyRowMapper<Passport>(Passport.class))).thenReturn(getPassport());
	List<Passport> num=passportDaoImpl.getAll(passportRequest);
