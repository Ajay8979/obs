 ...
 * 
 * @Ignore public class PassportDaoImplTest {
 * 
 * @InjectMocks PassportDaoImpl passportDaoImpl;
 * 
 ...
 * 
 * @Spy ResponseEntity<Object> conflict = new ResponseEntity<>(errorResponse,
 * HttpStatus.CONFLICT);
 * 
 * @Spy ResponseEntity<Object> sucessResponse = new
 * ResponseEntity<>(errorResponse, HttpStatus.OK);
 * 
 * @Spy PassportRequest passportRequest = new PassportRequest();
 * 
 * @Spy PassportResponse passportResponse = new PassportResponse(); int[] update
 * = { 1, 2, 3 }; boolean status;
 * 
 * @Before public void init() { passportDaoImpl = new PassportDaoImpl();
 * jdbcTemplate = mock(JdbcTemplate.class); setCollaborator(passportDaoImpl,
 ...
 * { e.printStackTrace(); } }
 * 
 * public PassportRequest passportRequest() { passportRequest = new
 * PassportRequest(); passportRequest.setPassportList(this.getPassport());
 * passportRequest.setPageNo(1); passportRequest.setPageSize(2);
 * passportRequest.setSessionId("1234"); return passportRequest; }
 * 
 * public List<Passport> getPassport() { List<Passport> list = new
 * ArrayList<Passport>(); Passport passport = new Passport(); passport.setId(1);
 * passport.setCenterName("centername1"); // passport.setCreatedBy(1234); // //
 * passport.setUpdatedBy(1234); //passport.setFlag(true); list.add(passport);
 * return list; }
 * 
 * @Test public void saveTest() throws SQLException { passportRequest = new
 ...
 * passportDaoImpl.savePassport(passportRequest); assertEquals(true, status);
 * 
 * }
 * 
 ...
 * 
 * @Test public void updateTest() throws SQLException { passportRequest = new
 * PassportRequest(); passportRequest.setPassportList(getPassport());
 * passportRequest.setTransaactionType("update");
 * passportRequest.setSessionId("1214");
 * when(jdbcTemplate.batchUpdate(Mockito.anyString(),
 * Mockito.anyList())).thenReturn(update); status =
 * passportDaoImpl.updatePassport(passportRequest); assertEquals(true, status);
 * }
 * 
 ...
 * }
 * 
 * // @Test public void getCountTest() throws SQLException { passportRequest =
 * new PassportRequest();passportRequest.setPassportList(getPassport()); int
 * count = 0 ; when(jdbcTemplate.
 ...
 * }
 * 
 * @Test public void getByIdTest() throws SQLException { passportRequest = new
 * PassportRequest(); passportRequest.setPassportList(this.getPassport());
 * passportRequest.getPassportList().get(0).setId(1);
 * 
 * Passport count = null; when(jdbcTemplate.
 * queryForObject("select * from ojas_obs.obs_passport where id = ?",
 * Passport.class)) .thenReturn(count); List<Passport> num =
 * passportDaoImpl.getById(passportRequest); boolean status = num.isEmpty();
