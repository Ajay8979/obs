...
public class StatesDAOTest {
	...
	public void init() {
		...
		setCollaborator(statesDAOImpl,"jdbcTemplate",jdbcTemplate);
	}
	...
	public void saveTest() throws SQLException {
		...
		statesRequest.setSessionId("1214");
		int[] count=  {1,2};
	 ...
	 * statesRequest.setSessionId("1214"); int[] count= {1,2};
	 * when(jdbcTemplate.batchUpdate(anyString(),anyList())).thenReturn(count);
	 ...
	 * statesRequest.setSessionId("1214"); int[] count= {0,0};
	 * when(jdbcTemplate.batchUpdate(anyString(),anyList())).thenReturn(count);
	 * boolean status=statesDAOImpl.deleteStates(statesRequest);
	 * assertEquals(false,status ); }
	 */
	...
	public void getAllTest() throws SQLException {
		statesRequest = new StatesRequest();
		statesRequest.setStates(getModel());
		when(jdbcTemplate.query("fghfgh",new BeanPropertyRowMapper<States>(States.class))).thenReturn(getModel());
		List<States>num=statesDAOImpl.getAll(statesRequest);
	...
	public void getAllCountTest() throws SQLException {
		...
		statesRequest.setStates(getModel());
		int num=4;
		when(jdbcTemplate.queryForObject("select count(*) from obs_states",Integer.class)).thenReturn(num);
		int count=statesDAOImpl.getAllStatesCount();
	...
	public void getAllCountPerPageTest() throws SQLException {
		...
		List<States>num=statesDAOImpl.getCountPerPage(getModel(),2,3);
		boolean status=num.isEmpty();
		assertEquals(false,status );
