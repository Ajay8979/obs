 ...
 * 
 * @Before public void init() { employeeCertificationDAOImpl= new
 * EmployeeCertificationDAOImpl(); jdbcTemplate= mock(JdbcTemplate.class);
 ...
 * certiRequest.setTransactionType("save"); certiRequest.setSessionId("1214");
 * 
 ...
 * }
 * 
 * @Test public void saveNegativeTest() throws SQLException { certiRequest = new
 * CertificationDetailsRequest();
 * certiRequest.setCertificationDetailsModel(getModel());
 * certiRequest.setTransactionType("save"); certiRequest.setSessionId("1214");
 * int[] count= {0,0,0};
 ...
 * CertificationDetailsRequest();
 * certiRequest.setCertificationDetailsModel(getModel()); int count=4;
 * when(jdbcTemplate.
 * queryForObject("select count(*) from obs_CertificationDetails where flag= true"
 * ,Integer.class)).thenReturn(count); int
 * num=employeeCertificationDAOImpl.getAllCertificationDetailsCount();
 * assertEquals(4,num );
 * 
 * }
 * 
 * @Test public void getAllCountPerPageTest() throws SQLException { certiRequest
 * = new CertificationDetailsRequest();
 * certiRequest.setCertificationDetailsModel(getModel()); int count=4;
 * when(jdbcTemplate.
 * queryForObject("select count(*) from obs_CertificationDetails where flag= true"
 * ,Integer.class)).thenReturn(count);
 * List<CertificationDetails>num=employeeCertificationDAOImpl.getCountPerPage(
 * getModel(),2,3); boolean status=num.isEmpty(); assertEquals(false,status );
 * 
 * }
 * 
 * 
